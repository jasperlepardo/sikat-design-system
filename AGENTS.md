...
- **Unit tests**: `src/**/*.test.{ts,tsx}`, Vitest project `unit` (jsdom). Run with `npx vitest --project unit`. Test files are excluded from the published `.d.ts` output.
- **Component tests**: all `.stories.tsx` files, Vitest project `storybook`. Run with `npx vitest --project storybook` (Playwright, Chromium headless).
